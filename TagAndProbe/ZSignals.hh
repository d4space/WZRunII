...
CBreitWignerConvCrystalBall::CBreitWignerConvCrystalBall(RooRealVar &m, const Bool_t pass)
{
  ...
  if(pass) {
    sprintf(vname,"mean%s",name);  mean  = new RooRealVar(vname,vname,0,-1,1);
    sprintf(vname,"sigma%s",name); sigma = new RooRealVar(vname,vname,1,0.1,2);
    sprintf(vname,"alpha%s",name); alpha = new RooRealVar(vname,vname,2,0,4);
    sprintf(vname,"n%s",name);     n     = new RooRealVar(vname,vname,0.1,0,1);
  } else {
